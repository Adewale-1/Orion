...
class Project:
    ...
    def add_dest_plates(self):
        ...
        for current_plate in self.plates_dest:
            ...
            for well_id in current_plate.structures_dict:
                ...
                ws_current[well_id] = current_plate.structures_dict[
                    well_id
                ].structure_name
                    ...
                    for rowNum in range(1, current_plate.col_length + 1, 1):
                        ...
                        if current_cell.value:
                            ...
                            if structure_name in self.structures_dict:
                                ...
                                # Current plate
                                current_plate.structures_dict[well_id] = (
                                    current_structure
                                )
    ...
    def assign_color_counters(self):
        ...
        for key in self.oligos_dict:
            self.oligos_dict[key].colorctr = self.color_counts[
    ...
    def assign_sortkeys(self):
        ...
        """
        for key in self.oligos_dict:
            self.oligos_dict[key].sortkey = (
                self.oligos_dict[key].colorctr,
                self.oligos_dict[key].color,
    ...
    def assign_bitseqs(self):
        ...
        """
        for key in self.oligos_dict:
            self.oligos_dict[key].bitseq = "".join(
                [str(x) for x in self.oligos_dict[key].bitlist]
            )
    ...
    def prepare_oligos_list(self):
        ...
        self.oligos_list.sort(
            key=lambda x: (x.length, x.vh5p, x.idx5p, x.vh3p, x.idx3p, x.sequence)
        )
    ...
    def parse_input_files(self, input_fnames):
        ...
        self.json_files = list(
            filter(lambda x: os.path.splitext(x)[1] == ".json", self.json_files)
        )
    ...
    def _write_info_sheet_96well(self):
        ...
        ws_current.append(
            ["Stock Concentration (nM)", "%.2f" % (self.stock_conc_96well)]
        )
    ...
    def _write_info_sheet_384well(self):
        ...
        # Calculate stock concentration
        self.stock_conc_384well = (
            self.oligoconc * self.ECHOdispense / self.ECHOvol
        )  # in nM

        # ECHO dispense volume
        ws_current.append(
            ["Stock Concentration (nM)", "%.2f" % (self.stock_conc_384well)]
        )
    ...
    def set_output_directory(self, output_directory=None, root_directory=None):
        ...
        if output_directory is None:
            self.output_directory = (
                head + "/" + root + "_autopipette_%03d" % (output_counter)
            )
        else:
    ...
    def prepare_ECHO_reservoir_input(self):
        ...
        for dest_plate in self.plates_dest:
            ...
            ordered_structures = collections.OrderedDict(
                sorted(dest_plate.structures_dict.items())
            )

            for well_id, current_structure in ordered_structures.items():

                dest_echo_input = {
                    "destPlate": dest_plate.plate_label,
                    "destWell": well_id,
    ...
    def sort_ECHO_input(self):
        ...
        """
        # Sort by source well
        self.echo_input = sorted(
            self.echo_input,
            key=lambda entry: (entry["sourceWell"][0], int(entry["sourceWell"][1:])),
        )
        # Sort by source plate
        self.echo_input = sorted(
            self.echo_input, key=lambda entry: entry["sourcePlate"]
        )
    ...
    def sort_ECHO_reservoir_input(self):
        ...
        """
        self.echo_res_input = sorted(
            self.echo_res_input,
            key=lambda entry: (entry["destWell"][0], int(entry["destWell"][1:])),
        )
        # Sort by source plate
        self.echo_res_input = sorted(
            self.echo_res_input, key=lambda entry: entry["destPlate"]
        )
    ...
    def read_plates_384well(self, fname, num_header=1):
        ...
        if os.path.isfile(fname):
            ...
            for row in ws_plates.iter_rows(min_row=num_header + 1, values_only=True):
                ...
                new_oligo.vh3p, new_oligo.idx3p = [
                    int(x) for x in match_finish.groups()
                ]
        ...
        for i in range(self.num_structures):
            ...
            for j in range(len(self.stock_keys)):
                ...
                if membership_key[i] == "1":
                    ...
                    # Stock row
                    current_structure.stock_row.append(
                        "%d (%d)" % (j + 1, self.stocks[current_stock_key].count)
                    )

                    # Water update
                    current_structure.water_96well -= self.stocks[
                        current_stock_key
                    ].count

                    # Add stock to structure
                    current_structure.stocks[current_stock_key] = self.stocks[
                        current_stock_key
                    ]

            # Add water
            current_structure.stock_row.append(
                "H2O (%d)" % (current_structure.water_96well)
            )
        ...
        for colNum in range(2, maxCol + 1, 1):
            for rowNum in range(2, maxRow + 1):
                ...
                if current_cell.value and current_cell.value[0] != "H":
                    ...
                    stock_color = openpyxl.styles.colors.Color(
                        rgb=self.stock_keys[stock_id][1][1:]
                    )

                    # Set color
                    current_cell.fill = openpyxl.styles.fills.PatternFill(
                        patternType="solid", fgColor=stock_color
                    )
            ...
            for well_id in current_plate.structures_dict:
                ...
                if current_structure.structure_name not in self.wb_384well:
                    ...
                    if current_structure.num_excluded > 0:
                        ws_current = self.wb_384well.create_sheet(
                            title=current_structure.structure_name
                        )
    ...
    def _write_plate_sheets_384well(self):
        ...
        for current_oligo in self.oligos_list:
            ...
            # Get plate id
            plate_label = "Plate%s-%d" % (
                self.plate_header,
                counter_plate.current_plate_id,
            )
    ...
    def _write_stocks_sheet_96well(self):
        ...
        for current_stock_key in self.stock_keys:
            self.ws_stocks.append(
                [
                    self.stocks[current_stock_key].stock_id + 1,
    ...
    def write_oligos_384well(self, out_fname):
        ...
        if self.ECHOdestfile is None:
            self._write_structure_sheet_384well(
                nreps=self.ECHOnreps, addspace=self.ECHOspace
            )
        else:
...
class Structure:
    ...
    def prepare_ECHO_input(self):
        ...
        for current_oligo in self.oligos_list:
            if current_oligo.include:
                ...
                # Prepare echo input
                self.echo_input.append(
                    {
                        "sourcePlate": plate_label,
                        "sourceWell": well_id,
                        "volume": volume,
    ...
    def prepare_ECHO_reservoir_input(self):
        ...
        self.water_384well = (
            self.water_384well - 1.0 * self.num_included * self.echo_drop_vol
        )

        # Prepare reservoir input
        self.echo_res_input = [
            {
                "sourcePlate": "Reservoir",
                "sourceWell": "A1",
    ...
    def is_crossover(self, current_key=None, neighbor_key=None):
        ...
        # Get neighbor list
        neighbor_lists, pairs = self.part.potentialCrossoverMap(
            current_idNum, current_idx
        )
        ...
        results = [
            item[0] == current_idx and neighbor_idx in item[list_index]
    ...
    def read_oligos(self, cadnano_part, scaffold_sequence, add_T=False, welding=False):
        ...
        self.cadnano_oligos = sorted(
            self.cadnano_oligos, key=lambda x: x.length(), reverse=True
        )
...
class Oligo:
    ...
    def make_sort_key(self):
        ...
        Make sort key
        """
        self.sortkey = (self.colorctr, self.color, self.bitseq)
...
def main():
    ...
    parser.add_argument(
        "-i", "--input", type=str, nargs="+", required=True, help="Cadnano json file(s)"
    )
    ...
    parser.add_argument("-seq", "--sequence", type=str, help="Scaffold sequence file")

    parser.add_argument(
        "-header", "--header", type=str, default="", help="Plate header"
    )

    parser.add_argument(
        "-offset", "--offset", type=int, default=None, help="Sequence offset"
    )

    parser.add_argument("-addT", "--addT", action="store_true", help="Replace ? with T")
    ...
    parser.add_argument(
        "-welding", "--welding", action="store_true", help="Add T's for welding"
    )
    ...
    parser.add_argument(
        ...
        "--read384",
        type=str,
        help="Read 384-well plate excel project file",
        default=None,
    )
    ...
    echo_output_384well_6res_to_96well = (
        new_project.output_directory + "/echo_384well_6res_to_69well.csv"
    )
    echo_output_384well_384PP_to_96well = (
        new_project.output_directory + "/echo_384well_384PP_to_69well.csv"
    )
    ...
    for i in range(len(json_files)):
        ...
        # Apply the offset
        new_structure.scaffold_sequence = (
            new_project.scaffold_sequence[new_structure.offset :]
            + new_project.scaffold_sequence[: new_structure.offset]
        )
        new_structure.oligos_dict = new_structure.read_oligos(
            part, new_structure.scaffold_sequence, args.addT, args.welding
        )
    ...
    new_project.write_ECHO_input(
        echo_output_384well_6res_to_96well, new_project.echo_res_input
    )
    new_project.write_ECHO_input(
        echo_output_384well_384PP_to_96well, new_project.echo_input
    )
