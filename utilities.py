...
def get_dH_SantaLucia2004(sequence):
    ...
    try:
        ...
        logging.error(f"Invalid sequence in get_dH_SantaLucia2004: {sequence}")
        return 0
...
def get_dS_SantaLucia2004(sequence):
    ...
    try:
        ...
        logging.error(f"Invalid sequence in get_dS_SantaLucia2004: {sequence}")
        return 0
...
def get_Tm_Mg_Owczarzy2008(sequence, seq_length, Tm_1M_NaCl, Mg_conc=Mg_CONC):
    ...
    Tm_Mg_inv = (
        ...
        + b * ln_Mg
        + fGC * (c + d * ln_Mg)
        + 1.0 / (2 * (seq_length - 1)) * (e + f * ln_Mg + g * ln_Mg**2)
    )
    Tm_Mg = 1.0 / Tm_Mg_inv
...
def get_min_scaffold_distance(
    scaffold_length, start_index, end_index, is_scaffold_circular=True
):
...
def end_to_end_distance(num_bases):
    ...
    return (
        ...
        * L_p
        * L_contour
        * (1 - 1.0 * L_p / L_contour * (1 - np.exp(-1.0 * L_contour / L_p)))
    )
...
def distance_to_loop_dG(distance_square, temperature_kelvin=323.15):
    ...
    effective_concentration = (
        1.0
        / 6.02e23
        * (3.0 / (2 * np.pi * distance_square * 1e-18)) ** (3 / 2.0)
        / 1000
    )
...
def position_to_loop_dG(
    scaffold_length,
    start_index,
    end_index,
    scaffold_circular=True,
    temperature_kelvin=323.15,
):
    try:
        # Get number of minimum number of bases between the two locations
        base_distance = get_min_scaffold_distance(
            scaffold_length, start_index, end_index, scaffold_circular
        )
        ...
        dGloop, dSloop = distance_to_loop_dG(distance_square, temperature_kelvin)

        # logging.debug(
        #     f"position_to_loop_dG: scaffold_length={scaffold_length}, start_index={start_index}, end_index={end_index}, base_distance={base_distance}, distance_square={distance_square}, temperature={temperature_kelvin}, dGloop={dGloop}, dSloop={dSloop}"
        # )

        return dGloop, dSloop
    except Exception as e:
        logging.error(f"Error in position_to_loop_dG: {e}")
        return 0, 0
...
def sequence_to_dG_dH_dS(sequence, temperature_kelvin=323.15):
    ...
    if seq_length == 0:
        logging.error("Sequence length is zero.")
        return 0, 0, 0
    ...
    try:
        ...
        if not all(base in valid_bases for base in sequence):
            logging.warning(f"Invalid bases in sequence: {sequence}")
            sequence = "".join(base for base in sequence if base in valid_bases)

        # Recalculate sequence length after cleaning
        seq_length = len(sequence)

        if seq_length < 2:
            logging.warning(f"Sequence too short after cleaning: {sequence}")
            return 0, 0, 0
        ...
        # logging.debug(f"dH total: {dHtotal}")
        # logging.debug(f"dS base: {dSbase}")
        # logging.debug(f"dS salt: {dSsalt}")
        # logging.debug(f"dS total (kcal/mol*K): {dStotal}")
        # logging.debug(f"dG total (kcal/mol): {dGtotal}")

        return dGtotal, dHtotal, dStotal
    except Exception as e:
        logging.error(f"Error in sequence_to_dG_dH_dS for sequence {sequence}: {e}")
        return 0, 0, 0
...
def conc_to_dG(temperature_kelvin, stap_conc=STAP_CONC, scaf_conc=SCAF_CONC):
    try:
        ...
        # logging.debug(
        #     f"conc_to_dG: temperature_kelvin={temperature_kelvin}, stap_conc={stap_conc}, scaf_conc={scaf_conc}, dSconc={dSconc}, dGconc={dGconc}"
        # )

        return dGconc, dSconc
    except Exception as e:
        logging.error(f"Error in conc_to_dG: {e}")
        return 0, 0
...
def sequence_to_dG_dH_dS(sequence, temperature_kelvin=323.15):
    ...
    # Change added
    # logging.debug(
    #     f"sequence_to_dG_dH_dS: sequence={sequence}, dHtotal={dHtotal}, dSbase={dSbase}, dSsalt={dSsalt}, dGtotal={dGtotal}"
    # )
...
# def svg_to_jpg(self, svg_file_path, jpg_file_path, quality=95):
#     logging.info(f"Converting {svg_file_path} to {jpg_file_path}")
...
#             logging.error(f"Error: Failed to convert {svg_file_path} to PNG.")
#             logging.error(f"Inkscape stderr: {result.stderr}")
#             return

#         # Check if PNG file was created
#         if not os.path.exists(png_file_path):
#             logging.error(f"PNG file was not created: {png_file_path}")
#             return

#         # Convert PNG to JPG using Pillow
#         image = Image.open(png_file_path)
#         rgb_im = image.convert("RGB")
#         rgb_im.save(jpg_file_path, quality=quality)

#         logging.info(f"Conversion successful: {svg_file_path} -> {jpg_file_path}")
