...
# import autobreak_main
import scaffolds as scaffolds
...
class Sequence:
    ...
    def assign_scaffold_positions(self):
        ...
        from autobreak_main import GroupBreaksolution
...
class OligoGroup:
    ...
    #                 new_group_solution.break_solutions[oligo.key] = random.choice(
    #                     oligo.break_solutions
    #                 )
    #             else:
    ...
    def combine_oligo_solutions(self, num_global_solutions=200):
        ...
        for i in range(num_global_solutions):
            ...
            for oligo in self.oligos:
                # Randomly pick one solution for an oligo
                if len(oligo.break_solutions) > 0:
                    chosen_solution = random.choice(oligo.break_solutions)
                    new_group_solution.break_solutions[oligo.key] = chosen_solution
                    if chosen_solution is None:
                        logging.warning(f"No solution chosen for oligo {oligo.key}")
                else:
                    new_group_solution.break_solutions[oligo.key] = None
                    logging.warning(
                        f"No break solutions available for oligo {oligo.key}"
                    )
    ...
    def create_stepwise_oligo_solutions(
        self,
        num_oligo_solutions=100,
        num_global_solutions=500,
        pick_method="random",
        shuffle_oligos=True,
        verbose=False,
    ):
        """Create stepwise oligo solutions"""
        from autobreak_main import GroupBreaksolution  # Deferred import
        from fastprogress.fastprogress import master_bar, progress_bar
    ...
    def sort_solutions(self, filter_incomplete=True):
        ...
        if filter_incomplete:
            self.group_solutions = list(
                filter(lambda x: x.complete, self.group_solutions)
            )
    ...
    def apply_break_rule(self):
        ...
        if "long" in self.break_rule:
            if self.length >= self.LONG_STRAND_LENGTH:
                ...
                fwd_breaks = list(
                    range(
                        self.LONG_STRAND_STEP - 1,
                        self.length - self.LONG_STRAND_STEP,
        ...
        self.rev_breaks = np.array(sorted(self.rev_breaks), dtype=int) + int(
            self.length
        )

        # All values need to be between 0 and length
        self.fwd_breaks = list(
            filter(lambda x: x >= 0 and x < self.length, self.fwd_breaks)
        )
        self.rev_breaks = list(
            filter(lambda x: x >= 0 and x < self.length, self.rev_breaks)
        )
        ...
        self.all_breaks = np.sort(
            np.unique(np.hstack((self.fwd_breaks, self.rev_breaks)))
        )
...
class Oligo:
    def __init__(self):
        ...
        self.num_solutions_per_oligo = 1
    ...
    def get_initial_score(self):
        ...
        from autobreak_main import BreakEdge  # Deferred import
    ...
    def color_by_csv_param(self, param="Tf", color_map="bwr"):
        ...
        cmap = plt.get_cmap(color_map, 1000)
    ...
    def color_by_folding_prob(self, color_map="bwr"):
        ...
        # Prepare the color map
        cmap = plt.get_cmap(color_map, 1000).reversed()
    ...
    def get_TfColor(self, Tf, min_Tf=30, max_Tf=70):
        ...
        # Get RGB value
        self.rgb = reversed_cmap(cmap_index)[:3]

        # Get hex-color
        hexcolor = matplotlib.colors.rgb2hex(self.rgb)
        return hexcolor
    ...
    def remove_penalized_solutions(self):
        ...
        self.break_solutions = list(
            filter(lambda x: x.self_penalty == 0, self.break_solutions)
        )
    ...
    def keep_best_break_solutions(self):
        ...
        self.break_solutions = list(
            filter(lambda x: x.score == self.max_score, self.break_solutions)
        )
    ...
    def generate_shortest_paths(self, num_solutions=1, verbose=False):
        ...
        if verbose:
            logging.info(
                f"Processing oligo: {self.key} Number of breaks: {len(self.breaks)}"
            )
        ...
        if self.circular:
            for current_break in self.breaks:
                ...
                shortest_k_paths = current_break.get_k_shortest_paths(
                    current_break, self.num_solutions_per_oligo, k_select
                )
                self.break_solutions += shortest_k_paths

                if not shortest_k_paths:
                    logging.warning(
                        f"No shortest paths found for break {current_break.key}"
                    )

        else:
            self.reset_break_paths()
            self.reset_break_order_ids(self.start_break, self.final_break)
            shortest_k_paths = self.start_break.get_k_shortest_paths(
                self.final_break, self.num_solutions_per_oligo, k_select
            )
            self.break_solutions += shortest_k_paths

            if not shortest_k_paths:
                logging.warning(
                    f"No shortest paths found for breaks between {self.start_break.key} and {self.final_break.key}"
                )
...
class Origami:
    def __init__(self):
        ...
        self.scaffolds = None
        self.idnums = None
        self.scaffold_sequence = None
        self.reverse_scaffold = False
    ...
    #     if self.set_circularize and not self.scaffolds[0].isCircular():
    #         if (
    #             strand5p.idNum() == strand3p.idNum()
    #             and abs(strand5p.idx5Prime() - strand3p.idx3Prime()) == 1
    #         ):
    ...
    def set_sequence_offset(self):
        ...
        if self.sequence_offset is None:
            sequence_distance = self.get_scaffold_distance(
                self.sequence_start_pos, self.current_start_pos
            )
    ...
    def read_oligo(self, oligo, oligo_type="staple"):
        ...
        for strand in generator:
            ...
            new_strand.idxLow, new_strand.idxHigh = (
                (new_strand.idx5p, new_strand.idx3p)
                if new_strand.forward
                else (new_strand.idx3p, new_strand.idx5p)
            )
            new_strand.direction = -1 + 2 * strand.isForward()
            new_strand.complement_strands = strand.getComplementStrands()[
                :: new_strand.direction
            ]
            new_strand.length = (
                new_strand.direction * (new_strand.idx3p - new_strand.idx5p) + 1
            )
        ...
        for oligo in self.oligos["staple"]:
            ...
            while current_strand:
                if not current_strand.final_strand:
                    ...
                    next_strand = (
                        current_strand.next_strand or oligo.null_strand.next_strand
                    )
                    ...
                    new_crossover.neighbor_key = (
                        new_crossover.vh3p,
                        new_crossover.idx + new_crossover.direction,
                        -new_crossover.direction,
                    )
        ...
        for oligo in self.oligos["staple"]:
            ...
            while current_strand:
                ...
                for strand in complement_strands:
                    ...
                    comp_idxLow, comp_idxHigh = (
                        (comp_idx5p, comp_idx3p)
                        if comp_forward
                        else (comp_idx3p, comp_idx5p)
                    )
                    ...
                    new_sequence.idx5p, new_sequence.idx3p = (
                        (new_sequence.idxLow, new_sequence.idxHigh)
                    ...
                    new_sequence.strLow = current_strand.direction * (
                        new_sequence.idx5p - current_strand.idx5p
                    ) + current_strand.get_inserts(
                        new_sequence.idx5p, current_strand.idx5p
                    )
                    new_sequence.strHigh = current_strand.direction * (
                        new_sequence.idx3p - current_strand.idx5p
                    ) + current_strand.get_inserts(
                        new_sequence.idx3p, current_strand.idx5p
                    )
                    ...
                    new_sequence.totalLength = (
                        new_sequence.strHigh - new_sequence.strLow + 1
                    )

                    # Assign sequence distance from 5' end of oligo
                    new_sequence.distance = (
                        new_sequence.strLow + current_strand.distance
                    )

                    # Get the sequence
                    new_sequence.dna = current_strand.dna[
                        new_sequence.strLow : new_sequence.strHigh + 1
                    ]
                ...
                current_strand.sequence_idxLows = np.array(
                    current_strand.sequence_idxLows
                )
            ...
            while current_strand:
                ...
                while current_sequence:
                    ...
                    if current_sequence.direction * (final_idx - start_idx) > 0:
                        ...
                        new_sequence.length = (
                            new_sequence.idxHigh - new_sequence.idxLow + 1
                        )
        ...
        for oligo in self.oligos["staple"]:
            ...
            if oligo.circular:
                previous_sequence.next_sequence = (
                    oligo.null_strand.next_strand.sequences[0]
                )
    ...
    def update_sequences_dna(self):
        ...
        for oligo in self.oligos["staple"]:
            ...
            for current_sequence in oligo.sequences:
                current_sequence.dna = current_sequence.strand.dna[
                    current_sequence.strLow : current_sequence.strHigh + 1
                ]
        ...
        for oligo in self.oligos["staple"]:
            ...
            while current_strand:
                ...
                for idx in range(idx5p, idx3p + direction, direction):
                    ...
                    new_nucleotide.dsDNA = self.is_dsDNA(
                        new_nucleotide.vh, new_nucleotide.idx
                    )
    ...
    def generate_break_points(self):
        """Generate break points"""
        from autobreak_main import BreakNode
        ...
        for oligo in self.oligos["staple"]:
            ...
            oligo.null_break.key = (
                oligo.null_break.vh,
                oligo.null_break.idx,
                oligo.null_break.direction,
            )
            oligo.null_break.current_nucleotide = self.get_current_nucleotide(
                oligo.null_break.key
            )
            oligo.null_break.next_nucleotide = self.get_next_nucleotide(
                oligo.null_break.key
            )
            oligo.null_break.sequence = (
                current_strand.sequences[0]
                if len(current_strand.sequences) > 0
                else None
            )
            ...
            oligo.null_break.insert = current_strand.get_inserts(
                oligo.null_break.idx, oligo.null_break.idx
            )
            ...
            while current_strand:
                ...
                for i in range(len(current_strand.all_breaks)):
                    ...
                    new_break.idx = (
                        current_strand.idx5p + current_strand.direction * break_position
                    )
                    new_break.vh = current_strand.vh
                    new_break.direction = current_strand.direction
                    new_break.distance = (
                        current_strand.distance + break_position_adjusted + 1
                    )
                    new_break.key = (new_break.vh, new_break.idx, new_break.direction)
                    new_break.current_nucleotide = self.get_current_nucleotide(
                        new_break.key
                    )
                    ...
                    new_break.insert = current_strand.get_inserts(
                        new_break.idx, new_break.idx
                    )
                    ...
                    new_break.sequence = (
                        current_strand.sequences[sequence_id]
                        if sequence_id >= 0
                        else None
                    )
    ...
    def disable_staple_crossovers(self):
        ...
        for key in self.crossovers:
            if (
                self.crossovers[key].break_node is None
                or self.crossovers[key].type == "scaffold"
            ):
                continue
    ...
    def disable_scaffold_crossovers(self):
        ...
        for key in self.crossovers:
            if (
                self.crossovers[key].break_node is None
                or self.crossovers[key].type == "staple"
            ):
                continue
            if self.crossovers[key].break_node.location == "internal":
                self.crossovers[key].break_node.dont_break = True
            ...
            for current_break in oligo.breaks:
                ...
                if current_crossover:
                    ...
                    if current_crossover.neighbor:
                        current_break.neighbor_break = (
                            current_crossover.neighbor.break_node
                        )
        ...
        while current_strand:
            ...
            for idx in range(idx5p, idx3p + direction, direction):
                ...
                if num_inserts == -1:
                    ...
                    self.key2scaffold[key] = list(
                        range(position - num_inserts, position + 1)
                    )[::-1]
    ...
    def get_oligos(self):
        ...
        self.cadnano_oligos = sorted(
            self.cadnano_oligos,
            key=lambda x: (
                x.length(),
                x.strand5p().idNum(),
                x.strand5p().idx5Prime(),
                int(x.strand5p().isForward()),
            ),
            reverse=True,
        )
    ...
    #         self.scaffold_sequence = utilities.generate_random_sequence(
    #             self.scaffolds[0].length()
    #         )
    def read_sequence(self):
        if self.scaffold_sequence is None:
            # If scaffold_sequence is not set, generate a random sequence
            self.scaffold_sequence = "".join(
                random.choice("ATCG") for _ in range(self.scaffolds[0].length())
            )
    ...
    def apply_sequence(self, offset=0):
        ...
        if len(self.scaffolds) > 0:
            self.scaffolds[0].applySequence(
                self.scaffold_sequence[offset:] + self.scaffold_sequence[:offset]
            )
