...
log_file = os.path.join(log_dir, "autobreak_main.log")
...
class OligoBreakSolution:
    ...
    def __init__(self):
        ...
        logging.debug("OligoBreakSolution initialized")
    ...
    def get_csv_rows(self):
        ...
        logging.debug(f"Calculated dsDNA length: {self.dsDNA_length}")
...
class AutoBreak:
    ...
    def set_break_rule(self, new_break_rule=["xstap", "all10"]):
        """Set break rule"""
        # logging.info("Break Rules Applied .... ")
        # logging.info(f"{[rule for rule in new_break_rule]}")
    ...
    def permute_scaffold_sequence_autobreak(self, nitr=100):
        ...
        if self.permute_sequence:
            ...
            if nitr < final_itr:
                final_itr = nitr
        for itr in tqdm(range(0, final_itr), desc="Permutation loop"):
    ...
    def optimize(self, edge):
        ...
        # print("PARSING THROUGH OPTIMIZE....")
        # print(f"Score: {score}")
        return score
    ...
    def get_shortest_path(self, final_break):
        ...
        while stack:
            ...
            for break_edge in valid_break_edges:
                ...
                logging.debug(
                    f"Considering edge: current_score={new_break.score}, edge_weight={break_edge.edge_weight}, new_score={new_score}"
                )
...
def calculate_and_print_initial_gibbs_free_energy(origami_instance):
    initial_energy = origami_instance.calculate_gibbs_free_energy()
    print(f"Initial Gibbs Free Energy: {initial_energy}")
...
def main():
    ...
    try:
        autobreak_instance, origami_instance = run(is_notebook_session)
        print("Script completed.")

        if autobreak_instance.best_complete_solution:
            gibbs_free_energy = (
                autobreak_instance.best_complete_solution.calculate_gibbs_free_energy()
            )
            print(f"Final Gibbs Free Energy of the best solution: {gibbs_free_energy}")
            # Add this line to explicitly print the Gibbs free energy value
            print(f"GIBBS_ENERGY:{gibbs_free_energy}")
        else:
            print("GIBBS_ENERGY:None")
    except Exception as e:
