...
def custom_showerror(title, message, width=400, height=200):
    global error_window
    if error_window is None or not error_window.winfo_exists():
    ...
    Label(error_window, text=message, wraplength=width - 20, padx=10, pady=10).pack(
        expand=True
    )
    Button(error_window, text="OK", command=error_window.destroy, padx=10, pady=5).pack(
        pady=10
    )
    error_window.lift()
...
def get_application_path():
    if getattr(sys, "frozen", False):
        # If the application is run as a bundle, `sys._MEIPASS` may be available in PyInstaller.
        try:
            application_path = sys._MEIPASS
        except AttributeError:
            # Fallback for Nuitka or if `_MEIPASS` is not set
...
class OrigamiApp:
    ...
    def __init__(self, root):
        ...
        try:
            ...
            if not os.path.isfile(self.picture):
                ...
                #     "Error", f"The file '{self.picture}' does not exist"
                # )
                custom_showerror(
                    "Error",
                    f"The file '{self.picture}' does not exist",
                )
            ...
            # print("Logo successfully loaded and set.")
        except Exception as e:
            # logging.error(f"Error loading the logo: {str(e)}")
            # messagebox.showerror("Error", f"Error loading the logo: {str(e)}")
            custom_showerror("Error", f"Error loading the logo: {str(e)}")
    ...
    def handle_exception(self, exc_type, exc_value, exc_traceback):
        ...
        # messagebox.showerror(
        #     "Error",
        #     "An unexpected error occurred. Please check the GUI.log file for details.",
        # )
        custom_showerror(
            "Error",
            "An unexpected error occurred. Please check the GUI.log file for details.",
    ...
    def show_intro_screen(self):
        ...
        try:
            ...
            # messagebox.showerror("Error", f"Failed to load intro image: {str(e)}")
            custom_showerror("Error", f"Failed to load intro image: {str(e)}")
    ...
    def upload_file(self):
        ...
        try:
            ...
            custom_showerror("Error", f"Failed to upload file: {str(e)}")
    ...
    def select_download_location(self):
        try:
            ...
            custom_showerror("Error", f"Failed to select download location: {str(e)}")
    ...
    def run_process(self):
        try:
            ...
            custom_showerror("Error", f"An error occurred: {str(e)}")
        finally:
            # logging.info(
    ...
    def run_optimization(self):
        try:
            ...
            custom_showerror("Error", f"Failed to run Optimization: {str(e)}")
    ...
    def estimate_runtime(self, space, sample_size=5):
        try:
            ...
            if not sample_times:
                # logging.error(
                #     "All sample runs failed. Please check your setup.")
                # messagebox.showerror(
            ...
            # messagebox.showerror("Error", f"Failed to estimate total runtime: {str(e)}")
            custom_showerror("Error", f"Failed to estimate total runtime: {str(e)}")
    ...
    def run_autobreak(self, params, is_final_run=False, is_initial_run=False):
        ...
        # Move these assignments before the try block
        ...
        try:
            ...
            # logging.info(f"autobreak_main.py stdout: {result.stdout}")
            # logging.info(f"autobreak_main.py stderr: {result.stderr}")

            # match = re.search(
            #     r"Final Gibbs Free Energy of the best solution: ([-\d.]+)",
            #     result.stdout,
            # )
            # Search for the Gibbs energy value in the output
            gibbs_match = re.search(r"GIBBS_ENERGY:([-\d.]+| None)", result.stdout)
            ...
            # else:
            #     # logging.error(
            #     #     f"Error: Couldn't extract Gibbs free energy for {params}")
            #     # messagebox.showerror(
            #     #     "Error", f"Error: Couldn't extract Gibbs free energy for {params}"
            #     # )
            #     custom_showerror(
            #         "Error", f"Error: Couldn't extract Gibbs free energy for {params}"
            #     )
            ...
            if gibbs_match:
                gibbs_value = gibbs_match.group(1)
                print(f"Gibbs energy value: {gibbs_value}")
                if gibbs_value != "None":
                    gibbs_free_energy = float(gibbs_value)
                else:
                    gibbs_free_energy = None
            else:
                print("Gibbs energy value not found in output")
                return None, time.time() - start_time, params

            run_time = time.time() - start_time
            return gibbs_free_energy, run_time, params
            ...
            # messagebox.showerror("Error", f"Timeout for parameters: {params}")
            custom_showerror("Error", f"Timeout for parameters: {params}")
            return None, 3600, params
        except Exception as e:
            # logging.error(f"An unexpected error occurred: {str(e)}")
            # logging.error("autobreak_main.py timed out after 1 hour")
            # messagebox.showerror("Error", f"An unexpected error occurred: {str(e)}")
            custom_showerror("Error", f"An unexpected error occurred: {str(e)}")
            return None, time.time() - start_time, params
        except subprocess.TimeoutExpired:
            # logging.error("autobreak_main.py timed out after 1 hour")
            return None, 3600, params
        # except Exception as e:
        # logging.error(f"Unexpected error in run_autobreak: {str(e)}")
        finally:
            if not is_final_run:
    ...
    def show_output_info(self):
        ...
        1. A folder named '{os.path.basename(self.output_directory)}' has been created in your chosen download location.

        2. Inside this folder, you'll find:

        inputs/
            - {self.input_tail}  (Original input file)
            - {os.path.basename(self.args_file)}  (Run parameters)
            - {os.path.basename(self.seq_filename)}  (Scaffold sequence used)

        intermediates/
            - {os.path.basename(self.results_heatmap_ortho)}  (Orthographic helix schematic)
            - {os.path.basename(self.results_heatmap_path)}  (Path Heatmap diagram)
            - {os.path.basename(self.results_plots)}  (Thermodynamic plots)
            - {os.path.basename(self.autobreak_log)}  (Debug and stderr log)

        outputs/
            - {os.path.basename(self.json_legacy_output)}  (Break solution applied)
            - {os.path.basename(self.results_report)}  (Composite of heatmap and plots)
            - {os.path.basename(self.results_excel_file)}  (Per-staple model calculations)
        ...
        # Remove leading whitespace from the multi-line string
        info_text = textwrap.dedent(info_text)
...
if __name__ == "__main__":
    try:
        ...
        # messagebox.showerror(
        #     "Critical Error",
        #     "A critical error occurred. Please check the GUI.log file for details.",
        # )
        custom_showerror(
            "Critical Error",
            "A critical error occurred. Please check the GUI.log file for details.",
...
# print(f"Path to autobreak_main.py: {resource_path('autobreak_main.py')}")


# This is just a regular commet

#  This aagsgasgkagskgakskjsjk
